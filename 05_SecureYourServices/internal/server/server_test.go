...
import (
	...
	"github.com/lottotto/proglog/internal/auth"
...
func TestServer(t *testing.T) {
	for scenario, fn := range map[string]func(t *testing.T, rootClient api.LogClient, nobodyClient api.LogClient, config *Config){
		"produce/consume a message to/from the log succeeeds": testProduceConsueme,
...
// 各テストケースを設定するためのヘルパー関数。まずはサーバが動作するローカルネットワークのアドレスに対して、リスナー(l)を作成
func setupTest(t *testing.T, fn func(*Config)) (
	...
	t.Helper()
	// 0を指定すると、自動的に空いているポートを指定してくれる。
	// アドレスについては証明書に従ったものでなければならない。
	...
	return rootClient, nobodyClient, cfg, func() { rootConn.Close(); nobodyConn.Close(); server.Stop(); l.Close(); clog.Remove() }
}

func testProduceConsueme(t *testing.T, client, _ api.LogClient, config *Config) {
