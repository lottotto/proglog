...
func setupTest(t *testing.T, fn func(*Config)) (client api.LogClient, cfg *Config, teardown func()) {
	...
	//サーバのコネクションを暗号化されていない方式で設定するオプション
	...
	return client, cfg, func() { cc.Close(); server.Stop(); l.Close(); clog.Remove() }
}

func testProduceConsueme(t *testing.T, client api.LogClient, config *Config) {
